...
| 5 | `fix/hlsl-type-aliases` | — | TODO | HLSL vector type aliases (`float2`→`vec2`, etc.) |
...
### 4. HLSL vector type aliases missing (FIXED)

**Wallpaper**: Ephemeral (2411455579)  
**Error**: `ERROR: 0:433: '' : syntax error, unexpected IDENTIFIER`  

**Root cause**: Wallpaper Engine shaders use HLSL-style type names (`float2`,
`float3`, `float4`, etc.) which are not valid GLSL. The `SHADER_HEADER` macro
block already translated many HLSL→GLSL constructs (e.g. `lerp`→`mix`,
`frac`→`fract`) but was missing the fundamental vector type aliases.

Line 433 of the Ephemeral fragment shader:
```
float rand_1_05(in float2 uv)
```
`float2` is an HLSL type equivalent to GLSL's `vec2`.

**Fix** (`ShaderUnit.cpp`):
- Added `#define` macros to `SHADER_HEADER` for all common HLSL type aliases:
  - `float2`→`vec2`, `float3`→`vec3`, `float4`→`vec4`
  - `float2x2`→`mat2`, `float3x3`→`mat3`, `float4x4`→`mat4`
  - `int2`→`ivec2`, `int3`→`ivec3`, `int4`→`ivec4`

**Impact**: Fixes Ephemeral (2411455579). Also provides forward-compatibility
for any other wallpapers that use HLSL type names directly instead of GLSL
equivalents.

**Files changed**: `ShaderUnit.cpp` (additive only — 9 new `#define` lines).
...
| 2411455579 | Ephemeral | `#require` + `hlsl_types` |
